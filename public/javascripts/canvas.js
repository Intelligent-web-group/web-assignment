...
window.chat.on('image', (prevX, prevY, currX, currY) => {
  console.log(prevX, prevY, currX, currY)
  drawOnCanvas(window.ctx, window.canvas.width, window.canvas.height, prevX, prevY, currX, currY, color, thickness);
})
 ...
 */
const initCanvas = (sckt, imageUrl) => {
  socket = sckt;
  let flag = false,
      prevX, prevY, currX, currY = 0;
  window.canvas = $('#canvas');
  window.cvx = document.getElementById('canvas');
  let img = document.getElementById('image');
  window.ctx = cvx.getContext('2d');
  if (!imageUrl) {
    console.log(localStorage.getItem(window.roomNo + '-img'))
    img.src = localStorage.getItem(window.roomNo + '-img');
  } else {
    img.src = imageUrl;
  }
  ...
  canvas.on('mousemove mousedown mouseup mouseout', (e) => {
    prevX = currX;
    prevY = currY;
    ...
    if (e.type === 'mousemove') {
      if (flag) {
        ...
        if (flag) {
          ...
          window.chat.emit('image', this.roomNo, prevX, prevY, currX, currY)
        }
      }
    }
  });
  ...
  img.addEventListener('load', () => {
    ...
    let poll = setInterval(function () {
      if (img.naturalHeight) {
        ...
        const imageData = cvx.toDataURL('image/jpeg')
        $.post('/add', {roomNo: window.roomNo, imageData}, (res) => {
        })
      }
    }, 10);
  });
